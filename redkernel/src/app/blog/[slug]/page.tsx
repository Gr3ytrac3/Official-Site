import { notFound } from "next/navigation"
import { Section } from "@/components/section"
import { posts } from "@/lib/content"

export default function PostPage({ params }: { params: { slug: string } }) {
  const post = posts.find((p) => p.slug === params.slug)
  if (!post) return notFound()

  return (
    <Section>
      <article className="mx-auto max-w-3xl text-[#e5e5e5]">
        <header className="mb-4">
          <h1 className="text-2xl font-semibold tracking-tight md:text-3xl text-[#e5e5e5]">{post.title}</h1>
          <div className="mt-2 flex items-center gap-3 text-xs text-[#e5e5e5]">
            <span>{new Date(post.date).toLocaleDateString()}</span>
            <span>•</span>
            <span>{post.readingTime}</span>
          </div>
          <div className="mt-2 flex flex-wrap gap-2 text-[#e5e5e5]">
            {post.tags.map((t) => (
              <span
                key={t}
                className="rounded-md border border-neutral-200 bg-white/80 px-2 py-0.5 text-xs text-neutral-700"
              >
                {t}
              </span>
            ))}
          </div>
        </header>
        <div className="prose prose-neutral max-w-none prose-p:my-3">
          {post.body.split("\n\n").map((para, idx) => (
            <p key={idx}>{para}</p>
          ))}
        </div>
      </article>
    </Section>
  )
}
